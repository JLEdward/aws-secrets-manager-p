...
def replicate_key(kms_key_id, replication_region, key_policy):
    """Retrieves the KMS key policy for the primary key and creates a replica
    ...
    try:
        kms_client.replicate_key(
            ...
            Description=(
                "KMS CMK used by the Secrets Manager Backup application."
            ),
            KeyId=kms_key_id,
            Policy=key_policy,
        ...
        logger.info(
            f"\"{kms_key_id}\" already exists in "
            f"\"{replication_region}\".  Skipping."
        )
